                ...
                <div class="card-body">
                    {!! Form::model($produk, ['method' => 'post', 'route' => ['admin.produk.update', $produk->id], 'enctype' => 'multipart/form-data']) !!}
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="judul" class="form-control" value="{{ $produk->judul }}" placeholder="Judul" autocomplete="off">
                            <i class="text-danger">{{ $errors->first('judul') }}</i>
                        </div>
                        <div class="form-group">
                            <label for="">Deskripsi</label>
                            <textarea name="deskripsi" class="form-control" id="editor" rows="5" placeholder="Deskripsi" autocomplete="off">{!! $produk->deskripsi !!}</textarea>
                            <i class="text-danger">{{ $errors->first('deskripsi') }}</i>
                        </div>
                        <div class="form-group">
                            <label for="">Gambar</label><br>
                            <img src="{{ url($produk->gambar) }}" width="80"><br>
